...
--	Broke down the data into three tables, converted dates on excel, and assign entry_id as primary key
...
--	QUESTION 1:
			--		Which counties had the highest amount of electrical vehicles? Also form a CTE from the previous query
...
--	WITH clause:
			--		Known as a Common Table Expression (CTE), form a temporary table to make queries from
			--		Assigned AS Update_EV_Table
			--		Used the previous query but removed ORDER BY clause beauce it doesn't work with a CTE
--	SELECT clause:
			--		Show everything
--	FROM clause:
...
--	OUTPUT:
			...
			--	Most EV are battery powered ranging from 67% to 80% across all 5 counties
			--	Car manufacturers are investing more into battery vs hybrid, trend for future
...
--	SELECT clause:
			...
			--	CASE statement to exclude null or 0 values
				--	Basically saying if value is not 0 then do the arithmitic of vehicle equipment type divided by EV_Total
...
--	QUESTION 3:
			--	How has EV Population for King, Snohomish, and Pierce counties change over time? Also what is their percentage of EV on their respective county population.
...
;
--	OUTPUT:
			--	Shows King, Pierce, and Snohomish battery, hybrids, total EV, and total vehicles, and EV percentage to population YTD
...
--	SELECT clause:
			...
			--	SUM EV / SUM Total for percentage, multiplied 100 for percentage and then ROUNDED to 2 decimals
--	FROM clause:
			--  Perform same JOINS from before using the primary key 'entry_id'
--	WHERE clause:
			--	County we us IN to include string values that match either King, Snohomish, or Pierce
--	GROUP BY clause:
			--	for our non aggregate columns we have to include in our GROUP BY statement
--	ORDER BY clause:
...
-- QUESTION 4:
			--	What is the fourth highest Electric Vehicle Population county that registered through Washington State Department of Licensing (DOL) of recent year?
...
--	OUTPUT:
			--	Displays Clark county as the 4th listed with Dense Rank of Electric Vehicle Totals within the whole population
...
--	WHERE clause:
			--	Set it from DENSERANK column equals to 4 therefore grabbing the 4th rank
